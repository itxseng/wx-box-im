...
export default {
	...
	methods: {
		...
		handleGroupMessage(msg) {
			...
			// 群消息置顶
			if (msg.type == enums.MESSAGE_TYPE.GROUP_TOP_MESSAGE) {
		...
		reconnectWs() {
			...
			this.userStore.loadUser().then(userInfo => {
				...
				// 5s后重试
				setTimeout(() => this.reconnectWs(), 5000)
			})
		},
		onReconnectWs() {
			this.reconnecting = false;
	...
	onLaunch(options) {
		...
		this.refreshToken(loginInfo).then(() => {
			...
			// #ifdef H5
			if (options.query.scan && options.query.userId) {
				// 扫描用户二维码,跳转到用户主页
				uni.navigateTo({
					url: "/pages/common/user-info?id=" + options.query.userId
				})
			} else if (options.query.scan && options.query.groupId) {
				// 扫描群二维码,跳转到群主页
				uni.navigateTo({
					url: "/pages/group/group-info?id=" + options.query.groupId
				})
			} else {
				// 默认跳转到聊天页
				uni.reLaunch({
