<template>
	<view class="page mine-edit">
		<nav-bar title="修改我的信息" back></nav-bar>
		<view class="form">
			...
			<view class="form-item" v-if="configStore.registration.mode.includes('phone')" >
				<view class="label">手机</view>
				<view v-if="userInfo.phone" class="value">{{userInfo.phone}}</view>
				<view v-else class="value" @click="onBindPhone">
					<text>去绑定</text>
					<uni-icons class="arrow" type="right" size="16"></uni-icons>
				</view>
			</view>
			<view class="form-item" v-if="configStore.registration.mode.includes('email')" >
				<view class="label">邮箱</view>
				<view class="value" v-if="userInfo.email">{{userInfo.email}}</view>
				<view v-else class="value" @click="onBindEmail">
					<text>去绑定</text>
					<uni-icons class="arrow" type="right" size="16"></uni-icons>
				</view>
			</view>
